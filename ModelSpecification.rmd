---
title: "Appendix 3: Simulation Model Specification"
output: pdf_document
---
...
Where parameters are not hardcoded into the model functions they are specified in `Parameters/Parameters.R`, and where relevant described in the text below.
...
The total number of observations was 19948, across 200 assemblages. The fitted assemblage random effect explained a variance of 0.03036, with 0.02912  residual variance remaining. The fitted values of the intercept was  0.46736 (SD= 0.01751 ) and the fixed effect of $a = 0.9$ was 0.15067  (SD =  0.02476), which were used to calculate the values in the main text. 


## Critical Speed of Climate Change

The critical climate change rate tests was conducted with `CritCC_Calculator.R`. The core response is the proportion of species that go extinct at series of rates of climate change. We tested 50 'years' of climate change at rates of 0 to 0.4, in steps of 0.025. Thesholding was removed, instead we looked at the proportion of species within each assemblage that fell below 1.0 across all nodes (other measures of 'extinction' gave similar results).
...
The trials of different sensitivities was conducted with `Peak_Sensitivity_Calc()`. The same assemblages were used and climate change is introduced at a rate of $v=0.28$, a rate chosen based on the results of the critical rate of climate change trials described above. 


The 'conservation intervention' was modeled as a +2 increase of the underling population growth rate ($R_{i,x,t}$) at particular sites. Sites were initially modified when they had an x-coordinate that fell within the conservation region that was determined for each species based on the location of the optimum, and an offset parameter $L$. As climate change progressed, this window of conservation intervention moved with the climate velocity.  Examples of growth curves modified in this way with different skew directions ($a$) and locatiosn of interventions ($L$) are given in the figure below:
...
To demonstrate that the general lack of impact of changing the sign of the $a$ parameter we show in the main text is not due the insensitivity of the outcomes to changes in the magnitude of $a$, we generated communities with $a$ values of -0.9, -0.7, -0.3,, 0.3, 0.7 and 0.9, and then tested the critical rate of climate change as described above. As visibile in the figure below, the results show a strong impact of $a$ on $v*$ that is approximately symmetric about $a$ =0. 
